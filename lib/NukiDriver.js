  ...
  onPair(session) {
    ...
    session.setHandler('query_bridge', async () => {
      ...
      try {
        ...
        let listResponse;
        try {
          const tokens = (new Nuki()).getTokenParams(token, false);
          const listUrl = 'http://' + targetBridge.ip + ':' + targetBridge.port + '/list?' + tokens;
            ...
            if (listResponse.headers.get('content-type').startsWith('application/json')) {
              ...
              if (listObj) {
                for (let i in listObj) {
                  if (this.nukiDeviceType.includes(listObj[i].deviceType)) {
                    foundDevices.push({
                      name: listObj[i].name,
                      data: {
                        id: listObj[i].nukiId,
                        deviceType: listObj[i].deviceType
                      },
                      settings: {
    ...
    session.setHandler('manual_pairing', async (bridgeAddress) => {
      ...
      try {
        ...
        let infoResponse;
        try {
          const tokens = (new Nuki()).getTokenParams(this.token, false);
          const base = 'http://' + bridgeAddress.address + ':' + bridgeAddress.port;
          const aURL = new URL('info', base);
          if (tokens) {
            for (let pair of tokens) {
              aURL.searchParams.append(pair[0], pair[1]);
            }
          }
          infoResponse = await fetch(aURL.toString(), {
            method: 'GET',
            timeout: 4000
          });
        }
          ...
          case 200:
            if (infoResponse.headers.get('content-type').startsWith('application/json')) {
              ...
              for (let i in bridgeInfo.scanResults) {
                if (this.nukiDeviceType.includes(bridgeInfo.scanResults[i].deviceType)) {
                  foundDevices.push({
                    name: bridgeInfo.scanResults[i].name,
                    data: {
                      id: bridgeInfo.scanResults[i].nukiId,
                      deviceType: bridgeInfo.scanResults[i].deviceType
                    },
                    settings: {
                      address: bridgeAddress.address,
